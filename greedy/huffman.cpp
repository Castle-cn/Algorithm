...
struct NodeType  //优先队列结点类型
{
    ...
    int weight;  //权值
    bool operator<(
        const NodeType& s) const {  //用于创建小根堆
...
void CreateHTree()  //构造哈夫曼树
{
         ...
         j++)  //构造哈夫曼树的n-1个非叶结点
    {
        ...
        e2 = qu.top();
        qu.pop();  //出队权值次小的结点e2
        ht[j].weight = e1.weight +
                       e2.weight;  //构造哈夫曼树的非叶结点j
        ht[j].lchild = e1.no;
        ht[j].rchild = e2.no;
...
void DispHTree()  //输出哈夫曼树
{
    for (int i = 0; i < 2 * n - 1; i++) {
        printf(
            "    data=%c, weight=%d, lchild=%d, rchild=%d, "
            "parent=%d\n",
            ht[i].data, ht[i].weight, ht[i].lchild,
            ht[i].rchild, ht[i].parent);
    }
}
int WPL()  //求WPL
{
